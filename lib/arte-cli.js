...
module.exports = {
    ...
    get: async (url, bucket, name, version, metadata, quiet = false, force = false) => {
        ...
        try {
            ...
            if (!quiet) progress.start(chalk.blue('Searching artifacts'), false);
            ...
            if (!quiet && artifacts.length > 0) console.log(createTable(['bucket', 'name', 'version', 'normalizedVersion', 'path', 'fileSize', 'lastUpdate', 'metadata'], {
                bucket: { title: 'BUCKET' },
                name: { title: 'NAME' },
    ...
    search: async (url, bucket, name, version, metadata) => {
        ...
        try {
            ...
            // Search for artifacts
            progress.start(chalk.blue('Searching artifacts'), false);
            const artifacts = await search(url, bucket, name, version, metadata);
            progress.stop(chalk.green(`Found ${artifacts.length} artifact(s)!`));
            if (artifacts.length > 0) console.log(createTable(['bucket', 'name', 'version', 'normalizedVersion', 'path', 'fileSize', 'lastUpdate', 'metadata'], {
                bucket: { title: 'BUCKET' },
                name: { title: 'NAME' },
                version: { title: 'VERSION' },
                normalizedVersion: { title: 'NORMALIZED VERSION' },
    ...
    delete: async (url, bucket, name, version, metadata, force = false) => {
        ...
        try {
            ...
            if (artifacts.length > 0) {
                console.log(createTable(['bucket', 'name', 'version', 'normalizedVersion', 'path', 'fileSize', 'lastUpdate', 'metadata'], {
                    bucket: { title: 'BUCKET' },
                    name: { title: 'NAME' },
                    version: { title: 'VERSION' },
                    normalizedVersion: { title: 'NORMALIZED VERSION' },
                    path: { title: 'PATH' },
                    fileSize: { title: 'FILE SIZE', formatter: filesize },
                    lastUpdate: { title: 'LAST UPDATE', formatter: data => moment(data).humanize() },
                    metadata: { title: 'METADATA', formatter: data => util.inspect(data, { breakLength: Infinity, colors: chalk.supportsColor.level == 0 || !chalk.enabled ? false : true }) }
                }, artifacts));
                ...
                if (shouldDelete) {
                    progress.start(chalk.blue('Deleting artifacts'), false);
